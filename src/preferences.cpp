...
namespace lbz
{
	namespace prefs
	{
		namespace defaults
		{
			...
			static constexpr bool submit_client_details = true;
		...
		cfg_bool submit_client_details(guids::submit_client_details, defaults::submit_client_details);
	...
	public:
		...
		BOOL OnInitDialog(CWindow, LPARAM)
		{
			...
			m_submit_client_details = GetDlgItem(IDC_SUBMIT_CLIENT_DETAILS);
			m_submit_client_details.SetCheck(prefs::submit_client_details.get_value());
			m_submit_client_details.EnableWindow(enabled);
		...
		uint32_t get_state() override
		{
			auto has_changed = [&]()
			{
				...
				if (m_submit_client_details.IsChecked() != prefs::submit_client_details.get_value()) return true;
		...
		void apply() override
		{
			...
			prefs::submit_client_details = m_submit_client_details.IsChecked();
		...
		void reset() override
		{
			...
			m_submit_client_details.SetCheck(prefs::defaults::submit_client_details);
	...
	private:
		CCheckBox m_check_enabled, m_check_library, m_submit_client_details, m_check_skip, m_check_artist_first;
